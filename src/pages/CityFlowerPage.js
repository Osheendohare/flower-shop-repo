// src/pages/CityFlowerPage.js

import React from 'react';
import { useParams, Link } from 'react-router-dom';
import { cityFlowers } from './cityFlowers';
import { sentiments } from './OccasionSuggestions';
import { Container, Row, Col, Card } from 'react-bootstrap';

const CityFlowerPage = () => {
  const { cityName } = useParams();
  const flowerCategories = cityFlowers[cityName.toLowerCase()] || [];

  // Combine all flowers from sentiments and bestSelling
  const sentimentFlowers = Object.values(sentiments).flat();
  const allFlowers = [...sentimentFlowers];

  // Filter flowers matching city's categories
  const citySpecificFlowers = allFlowers.filter(flower =>
    flowerCategories.includes(flower.category)
  );

  return (
    <Container className="mt-4">
      <h2 className='mt-4 pd-4 mb-4' style={{textAlign:'center', fontWeight:'bolder' }}>Flowers Popular in {cityName}</h2>
      {citySpecificFlowers.length === 0 ? (
        <p>No flower suggestions found for {cityName}.</p>
      ) : (
        <Row>
          {citySpecificFlowers.map((flower, index) => (
